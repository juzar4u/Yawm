...
(function ($) {
    ...
    var capabilities = config.options.capabilities || new Array('select', 'delete', 'replace');
    ...
    var selectItem = function (data) {
        if (config.options.relPath !== false) {
            ...
            var url = data['Path'].replace(fileRoot, "");
        } else {
...
jQuery('#txtSearch').keyup(function () {
    ...
    if (event.keyCode == 27) {
        ...
        // hide all rows
        var ull = jQuery('#fileinfo ul').attr('id');
        jQuery('#' + ull + ' li').hide();

        jQuery('#' + ull + '').find('li > p').each(function () {
            var str = $(this).text();

            if (str.toLowerCase().indexOf(jQuery('#txtSearch').val().toLowerCase()) >= 0) {
                $(this).parent().show();
            }
        });
    }
    ...
    var ull1 = jQuery('#fileinfo ul').attr('id');
    // if there were no matching rows, tell the user
    if (jQuery('#' + ull1 + ' li:visible').length == 1) {
