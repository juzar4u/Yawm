...
namespace AkhbaarAlYawm.Application.Helper
{
    ...
    public class EmailService
    {
        ...
        private void GetFinalEmailBodyAndSubjectForUser(EmailTokens emailSimpleToken, PetaPoco.Database context)
        {
            ...
            if (template.Name.Equals("UserToUserEmail"))
            {
                emailSimpleToken.EmailBody = GetFinalEmailBodyForUserMessages(template.Body, emailSimpleToken.UserMessageID, emailSimpleToken.FromName, emailSimpleToken.EntityID);
            }
        }
        ...
        private string GetFinalEmailBodyForUserMessages(string body, int? userMessageId, string from, int userId )
        {
            using (PetaPoco.Database context = DataContextHelper.GetCPDataContext())
            {
